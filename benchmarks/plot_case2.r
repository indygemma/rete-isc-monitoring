...
data <- read.table(file="case2.csv", header=T, sep=",")

ordered_by_runtime <- data[with(data, order(-runtime)),]

head(ordered_by_runtime)
...
#nonnative_activation_chart <- plot_grouped_line_chart(nonnative_activation_data, "existing_events_n", "value", "label",
    #"(b) node activations (events N=1000, non-native event method)",
    #"# of existing events",
    #"# of activations",
    #"Node activated",
    #activation_colors)
